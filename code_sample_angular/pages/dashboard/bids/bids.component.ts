...
export class BidsComponent implements OnInit {
  ...
  bidStatus;
  totalOrders;
  totalBidsList = [{ is_flagged: true, user_id: '' }, { is_flagged: true, user_id: '' },
  { is_flagged: true, user_id: '' }, { is_flagged: true, user_id: '' }];
  constructor(private route: ActivatedRoute, private dashboardService: DashboardService) { }
  ngOnInit() {
    this.bidStatus = this.route.snapshot.routeConfig.path.split('/')[0]
  ...
  consumerListTable() {
    ...
    $('#datatables').DataTable({
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        that.totalBidsList.length = 0;
        const limit = 10;
        let fnName = 'getActiveBids';
        if (this.bidStatus === 'accepted_bids') {
          fnName = 'getAcceptedBids';
        }
        that.dashboardService[fnName](page, column, dir, this.state, this.time).subscribe(resp => {
          this.totalOrders = resp.meta.total_count
          that.noDataStyle = true;
          that.totalBidsList = resp.data;
      ...
      columns: [{ data: 'order_id' }, { data: 'provider_name' }, { data: 'consumer_name' }, { data: 'amount' },
      { data: 'order_type' }
      ]
    });
  }
